...
// function to retrieve all courses from the user table database
export const getCoursesByUserId = async (userId) => {
  try {
    // Call the API Gateway to get courses by user ID
    const response = await axios.get(`${API_GATEWAY_URL}/users/${userId}/courses`);
    return response.data;  // Assuming response contains an array of course IDs
  } catch (error) {
    console.error('Error fetching courses by user ID:', error);
    throw new Error('Failed to fetch courses by user ID');
  }
};
...
// // Uncomment this function if it's available in your API
